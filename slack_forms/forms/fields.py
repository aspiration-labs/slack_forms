...
class Field:
    ...
    def __init__(
        self, *, action_id: Optional[str] = None, block_id: Optional[str] = None
    ):
    ...
    def bind_field(self, state_values: Dict[str, Any]) -> "Field":
        try:
            if self.block.has_state:
                self._value = self.block.value_from_state(
                    state_values[self.block_id][self.action_id]
                )
            else:
                self._value = None
        except KeyError:
            raise BindError(
                f"{self.__class__.__name__} cannot find state for "
    ...
    def value(self) -> Optional[Union[str, List[str]]]:
        if not self.is_bound:
            raise BindError(
                f"{self.__class__.__name__} value is not bound; did you create the form with state?"
            )
        return self._value
